...
**이 수는 모델이 인용을 안 지킨 비율이 아니다.** 대조 대상인 fitz 텍스트도 원문이 아니라 두 번째 추출기의 출력이라, 둘이 다를 때 어느 쪽이 원문인지 이 대조는 모른다. 여덟 칸 전부와 근거는 [docs/variance.md](docs/variance.md) 「세 번째 층」에 있다. 읽기 순서가 왜 그렇게 움직이는지는 [pdf-read-order](https://github.com/serenwoon/pdf-read-order) 에서 따로 쟀다.
