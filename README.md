...
**이 수는 모델이 인용을 안 지킨 비율이 아니다.** 대조 대상인 fitz 텍스트도 원문이 아니라 두 번째 추출기의 출력이라, 둘이 다를 때 어느 쪽이 원문인지 이 대조는 모른다. 여덟 칸 전부와 근거는 [docs/variance.md](docs/variance.md) 「세 번째 층」에 있다. 읽기 순서가 왜 그렇게 움직이는지는 [pdf-read-order](https://github.com/serenwoon/pdf-read-order) 에서, 공백을 지운 칸이 무엇을 감췄는지는 [pdf-space-guess](https://github.com/serenwoon/pdf-space-guess) 에서 따로 쟀다. 뒤엣것이 이 격자의 읽기 하나를 뒤집는다 — 550 에서 258 로 떨어진 292 칸을 「원래 붙여 쓴다」로 읽었는데, 글자는 벌어져 있었고 fitz 가 공백을 안 내놓은 것이었다.
